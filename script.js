...
const ui = {
    ...
    renderSchedule() {
        ...
        // Optimization: Index bookings by machine
        const bookingsByMachine = {};
        allBookings.forEach(b => {
            if (!bookingsByMachine[b.machineType]) {
                bookingsByMachine[b.machineType] = [];
            }
            bookingsByMachine[b.machineType].push(b);
        });
        ...
        Object.keys(logic.machines).forEach(machineKey => {
            ...
            const machineBookings = bookingsByMachine[machineKey] || [];

            slots.forEach(slot => {
                const slotMins = utils.timeToMins(slot);
                const nextSlotMins = slotMins + 30;

                const booking = machineBookings.find(b => {
